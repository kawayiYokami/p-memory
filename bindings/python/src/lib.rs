...
impl NativeKnowledgeBase {
    ...
    /// 注册重排回调：库在调用前按声明的定长约束强制截断。
    #[pyo3(signature=(callback,max_docs=64,max_tokens_per_doc=1024,max_tokens_query=None))]
    fn register_reranker(&self,py:Python<'_>,callback:Py<PyAny>,max_docs:usize,
                         max_tokens_per_doc:usize,max_tokens_query:Option<usize>)->PyResult<()>{
        let kb=self.inner.clone();
        let reranker=PyReranker{callback};
        py.detach(move ||kb.register_reranker_with(reranker,
            RerankerOptions{max_docs,max_tokens_per_doc,max_tokens_query})).map_err(native_error)
    }
}
#[pyfunction]
