...
结果合并时**严格轮优先**：先取严格命中，再用宽松结果补齐未出现的记录，最后按组内分数与 `key` 排序。写入索引前，文本先经统一的 `clean_markdown` 清洗掉标记符（`body` 列照旧保留原文）。索引侧 schema 为 `key/namespace/scope/kind/tags/tags_text/text/body`：

- `text`（正文列）：这条记录自己的文本——记忆是 `judgment`，切片是它那一段。用预切分的空白分词器（`pretokenized`）承担 1+2 分词召回。
- `tags_text`（标签文本列）：每个标签一个词项（`STRING`），与正文列吃同一批查询词。BM25 要算分就得有词项，所以标签文本在索引里必须留一份，不能只有 id。
...
**正文列与标签文本列各算一次分、同一条记录内相加**：查「澜川」时，标签里带 `澜川` 的切片从标签文本列拿一份分，正文里也写着它的再加一份。RRF 只用在全文路与向量路之间，与记录内相加是两回事。

笔记记录不进索引：它自己没有正文，路径信息以标签形态挂在它的每个切片上。要文件列表就按库里的标签翻笔记（`notes().list` 带 `filter.tags`）。
...
| 纯全文 | 该领域向量未补齐（`vector_ready` 标记缺失） | `vector_not_ready` |
...
- 向量路径按 `row.key.namespace`、`row.scope`、`kinds`、`row.tags` 在打分前过滤。这里的 `kinds` 是「请求要的记录类型」与「该领域启用了向量化的类型」的交集：某档关掉后，它的存量向量不参与打分；交集为空时整条向量路不走（总闸关闭时同理，并记 `namespace_disabled`）。此外先看该领域在当前向量空间下是否**就绪**（`meta` 里的 `vector_ready:<领域>:<空间>`）：没就绪说明这个领域还有切片没算完向量，此时整条向量路不走、只给全文，并记 `vector_not_ready`——避免拿「只补了一半」的向量去和全文融合，让先补完的那部分凭空占位。
