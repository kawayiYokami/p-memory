...
flowchart LR
    ...
    E -->|是| F["两路候选交替合并、按 max_docs 截、按 token 预算截文本 → 重排"]
    E -->|否| G["RRF 融合"]
    F --> H["按重排分排序、截断 limit"]
    G --> H
```
...
6. **重排优先**：注册了重排回调且 `rerank=true` 时，两路候选（全文 + 向量）按**名次交替合并、去重**（全文第 0 名、向量第 0 名、全文第 1 名……），**不经 RRF**，直接按 `max_docs` 截断、文档与查询词按 token 预算截断后交给回调；重排分决定名次。RRF 不参与候选取舍——否则没有双路加持的真相关候选会在进重排之前就被砍掉。
...
9. **切片折叠**：同一篇笔记的多个命中切片只保留**排名最高的一片**，否则一篇对话体文档会用自己的几十个片段占满整个列表、把别的文档全挤出去。折叠后每条切片带上 `note_chunks`——这一篇命中本次查询的切片**总数**（含本条），它是在索引里按笔记精确统计出来的，与结果窗口、翻页无关。非切片命中、或请求只看名字/目录列（笔记级、一篇至多一条）时为 `None`。`total`（开启 `with_total` 时）仍是过滤后的记录数，不因折叠而变。截断到 `limit` 在折叠之后。
...
- `path`（目录列）：笔记相对路径里除文件名之外的各段（空格连接），只挂在这一篇的第一片。目录段**不进** `text`——否则搜「璃月」会命中该目录下每一篇；它只在一列上，供「书名块不够」的兜底查询（`match_field=path`）使用，常规检索（`all` / `text` / `name`）不查它。目录段仍写在 `record_tags` 里，按文件夹筛笔记的能力不变，只是不再参与匹配。
- `note`（笔记列）：切片所属笔记的记录 id。只挂在切片上，用来精确统计「这一篇命中本次查询的切片总数」（`note_chunks`），常规检索不查它。
...
索引里没有单独的标签文本列：标签集拼在承载它的那条正文前面，进的是同一条 `text` 列，跟着这条的文档长度一起被 BM25 的长度归一化。查「澜川」这类**文件名**时，它已迁到 `name` 列单列命中——那一份分不再被它自己那两百多字的正文稀释，也不再因为文档短而虚高。RRF 只用在全文路与向量路之间，与这里的一切无关。

笔记记录不进索引：它自己没有正文，文件名与目录段分别以 `name` / `path` 列写进它第一片切片文档（库里 `record_tags` 仍带路径段）。要文件列表就按库里的标签翻笔记（`notes().list` 带 `filter.tags`）。

## 谓词等价词的查询期扩散

关系写法常有同义多种（`丈夫` / `老公` / `夫君`）。BM25 拆字后跨不过同义，库里写 `丈夫`、查询说 `老公` 就对不上。库提供一张**按知识领域登记**的谓词等价词表，把这类写法归成一组；查询时自动扩散。
...
- 扩散：查询词里出现任一登记词（如 `老公`），就把整组同义词（`丈夫`、`夫君`）补进查询。
...
struct NoteSection {
    ...
    contents: Vec<SearchHit>,     // 内容块：切片正文命中（text 列），沿用本次请求的全文+向量+重排；同一篇只留最高的一片，note_chunks 报出这一篇的片段总数
