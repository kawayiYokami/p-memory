...
8. **排序截断**：重排分（重排可用时）或 RRF 分降序，同分按 `RecordKey` 升序，取前 `limit`。`score` 字段始终是 RRF 融合分，供诊断与兜底排序参考。
...
结果合并时**严格轮优先**：先取严格命中，再用宽松结果补齐未出现的记录，最后按组内分数与 `key` 排序。写入索引前，文本先经统一的 `clean_markdown` 清洗掉标记符（`body` 列照旧保留原文）。索引侧 schema 为 `key/namespace/scope/kind/tags/text/name/path/body`：
...
struct NoteSection {
    ...
    contents: Vec<SearchHit>,     // 内容块：切片正文命中（text 列），沿用本次请求的全文+向量+重排
