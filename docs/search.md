...
- `name`（名字列）：只有实体的规范名进来，其它记录为空、不写这一列。规范名不再拼进 `text`——正文列只装别名、摘要与属性，名字单独成列、独占匹配。查询侧对名字列命中按固定倍数（`NAME_FIELD_BOOST = 3.0`）加权，并且名字命中本身就是一个独立的命中条件：只靠规范名、正文为空的实体也能被搜到。名字整段等于查询词的记录因此不会被它那几百字的别名与属性摊薄。它治的是「正主被自己的别名与属性挤下去」这一类。
...
- 接入点：全文路（普通 `search` 与预设检索的实体/笔记两路）与预设检索图谱路的第二步，都用同一套扩散。
