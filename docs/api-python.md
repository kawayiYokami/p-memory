...
- 检索时宿主只给 `embed_space`，库用它注册的回调嵌入查询词；`search` 的 `text` / `vector` / `rerank` / `with_total` 各自独立开关，`match_field` 限定全文路在哪一列命中（`all` / `text` / `name` / `path`），未给 `embed_space` 时向量路自动让位（不是错误）。切片命中会折叠：同一篇笔记只留排名最高的一片，每条切片带 `note_chunks`——这一篇命中本次查询的片段总数（与结果窗口、翻页无关）。
