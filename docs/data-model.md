...
- 写入时文件缺失或非 UTF-8 直接报错，不落库；索引侧补切片文档时单个文件读不到只让那批切片正文退化为空，不让整次对账失败。
- 切片规则见 [笔记切片](#笔记切片)。
...
- 检索的降级只到纯全文为止，不设比 BM25 更弱的检索档；索引查询失败即隔离文本路（向量路照常，记 `text_index_unavailable`），不触发任何重建。索引目录打不开时在打开阶段只隔离那一份派生索引，再由对账把缺的补齐。
