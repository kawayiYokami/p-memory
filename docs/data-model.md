...
- 标题由路径文件名派生。文件名与目录名分别以 `name` 列与 `path` 列的形态进索引（见上）：两者都只挂在**第一片**上。搜文件名时，`澜川.md` 这一篇的第一片按名字列命中、带固定倍数加权，不再被自己那两百多字的正文稀释；目录段则只在「书名块不够」的兜底查询里被查，正文里写着「澜川」的切片各自按正文算分。
