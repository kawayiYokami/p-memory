...
- `path` 必填。领域必须在 `namespace_roots` 里登记过根目录（`set_root`），写入路径必须是它的子路径：库里存的是**减掉根目录的相对路径**（`bwiki/沧州/澜川.md`），路径不在根目录之内直接报校验错，不做猜测；**没登记根目录的领域不允许写入笔记**。`UNIQUE(namespace_id, scope_id, path)` 用来「同一文件重复同步定位到同一条」——路径一变身份就变，登记根目录前后同一个文件会被当成两篇，旧记录不自动改写。
- **相对路径拆成「目录段 + 文件名」**：`bwiki/沧州/澜川.md` → 目录段 `bwiki`、`沧州`，文件名 `澜川`（目录段原样，文件名去扩展名）。目录段仍与调用方给的标签合并去重，写在**每一条切片记录**上（库里 `record_tags` 写 `tag_id`），供按标签筛切片。进索引时，目录段进独立的 `path` 列、文件名进 `name` 列，都只挂在**第一片**上（见 [search](search.md)）；目录段不再拼进可搜正文，所以搜目录名不再命中该目录下每一篇。要文件列表按库里的标签翻笔记。
...
- **文件名与路径都在 `notes` 表存一次**：`path` 既用来读文件、也用来定位同一条，`name` 是写入时用 `Path::file_stem` 从路径取下的标题，`payload` 里两者都不重复存。文件名在写入那一刻就定下来，索引重建直接读它、不再事后拆路径。
...
- 标题是写入时存下的 `notes.name`。文件名与目录名分别以 `name` 列与 `path` 列的形态进索引（见上）：两者都只挂在**第一片**上。搜文件名时，`澜川.md` 这一篇的第一片按名字列命中、带固定倍数加权，不再被自己那两百多字的正文稀释；目录段则只在「书名块不够」的兜底查询里被查。正文里写着「澜川」的切片各自按正文算分；同一个查询下，同一篇笔记的多个命中切片在结果里**折叠成一条**（只留排名最高的一片），该条带 `note_chunks`——这一篇命中该查询的切片总数（见 [search](search.md)），让一篇对话体文档不至于用自己的几十个片段占满整个列表。
...
- 切片正文只存在索引里，所以切片向量只能在索引提交之后算：补齐自己会先把索引追平再取正文，取不到正文的记录跳过，**绝不用空文本凑一个向量**。
...
- **补齐（`embeddings().sync`）**是「库拿该空间注册的回调，把缺失向量补上」这一次动作，顺序固定：先把索引追平（切片正文只存在索引里，不追平就取不到字）→ 再按缺口分批调模型、短事务写回 → 最后逐档核对缺口。命中切口为 0 的档才被标成**就绪**。
- **就绪精准到功能档**：记忆、图谱、笔记三档各自独立就绪，互不牵连。某一档就绪 = 它在该领域（namespace × 向量空间）下所有应向量化的记录都已有当前指纹的向量。标记落盘在 `meta` 的 `vector_ready:<领域>:<空间>:<档位>`，键在即就绪。
- **触发是主动的，四个时机**：开库时、注册或切换模型时、切片（记录）写入提交后、以及一轮补齐跑完但仍有缺口时。线程平时睡在条件变量上，只在被叫的时候醒，不轮询。
...
- **核对不占写锁**：逐档查缺口在只读连接上做，写锁只用于最后落标记——否则写入与读取的自愈都会被它挡住。
