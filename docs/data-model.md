...
- **补齐（`embeddings().sync`）**是批次结束后由调用方触发的一次动作，顺序固定：先把索引追平 → 再按缺口分批调模型、短事务写回 → 最后逐领域核对缺口。命中切口为 0 的领域才被标成**就绪**。
- **就绪**的单位是「领域 × 向量空间」（namespace × space），标记落盘在 `meta` 的 `vector_ready:<领域>:<空间>`。键在即就绪，读不到就是没就绪。
- **应向量化**按该领域已启用的档位算：关掉的档位既不生成向量，也不计入缺口。
- **就绪标记的失效**：任何影响该领域「应向量化集合」的动作都让标记当场作废——写入、删除记录，以及改总闸或改档位。作废之后必须重新补齐核对过，才会再被标记。
- **检索门禁**：未就绪的领域整条向量路不走，只给全文，并记 `vector_not_ready`。补齐之后自动放行，不需要再动开关。这样可以保证参与打分的向量永远是「整个领域都补齐的那一份」，不会偏向先补完的那部分。
- **断点续传**：补齐可中断、可分多次，每次从缺口处接着走；已写回的批保留，未跑的批不写。中断即未就绪，重启后再补。
- **库内线程是兜底**：开库时启动、关库时停下并等它收尾。它每 2 秒看一次，只在「这一拍里有新写入、且这一拍之内没有新写入」时动手补齐——前者说明有活可干，后者说明这批写完了。它只写 `embeddings` 表，不写业务记录、不改索引。
- 调用方在批次结束调 `sync` 是主路径；线程兜住「忘了调」的情形。
