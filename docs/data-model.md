...
- **相对路径按段拆成标签**：`bwiki/沧州/澜川.md` → `bwiki`、`沧州`、`澜川`（目录段原样，文件名去扩展名），与调用方给的标签合并去重。这份标签写在**每一条切片记录**上（库里 `record_tags` 写 `tag_id`），供按标签筛切片；进索引可搜的那一份只拼在**第一片的正文前面**，索引里没有单独的标签列。要文件列表按库里的标签翻笔记。
...
- 标题由路径文件名派生。文件名与目录名以**标签**的形态存在（见上）：它只拼在这一篇第一片的正文前面，与这一片正文同列、同长度。搜「澜川」时，`澜川.md` 这一篇只有第一片带这份标签，而且这一份分被第一片的正文稀释；正文里也写着「澜川」的切片各自按正文算分。
