...
| `kb.graph().delete_predicate_rule(pred)` | 撤销一条对称/逆谓词元规则 | — |
| `kb.graph().delete_predicate_equivalents(namespace, predicates)` | 撤销等价登记；不给词就清整域 | — |
...
```rust
// 乙 --sys:same_as--> 乙先生 --导师--> 导师
let view = kb.graph().build_graph(&filter)?;
view.path_ids(lin_zhao, mentor);   // 折叠后不再夹着「乙先生」中间节点
