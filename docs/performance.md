...
| `1a388eb` | 删除路径不通知索引：删掉的记录、笔记改写后废弃的切片仍留在索引里，继续参与打分、占住候选配额 | sync 时对账：库里已查不到的记录按 key 从索引摘掉 |
...
**索引格式跟着变。** `3a90a6d` 把 `key` 改成整数快字段，格式升到 `p-memory-text-v12`；`17b5a85` 把 `note` 列改成索引加快字段，升到 `v13`。旧格式索引在下次打开时被隔离，由对账补齐一次。
