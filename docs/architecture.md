...
- 笔记的路径与文件名都在 `notes` 表存一次；领域必须登记过根目录（`namespace_roots`）才允许写入，路径存的是减掉根目录的相对路径，文件名是写入时用 `file_stem` 取下的 `name` 列。路径拆成目录段与文件名后分别落在索引的 `path` 列与 `name` 列上，只挂在这一篇第一片；目录段不参与常规匹配，只在「书名块不够」的兜底查询里被查。路径段仍写在每一条切片记录上供筛选。
