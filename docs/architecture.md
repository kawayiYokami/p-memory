...
flowchart TD
    ...
    D --> IX["Tantivy text-v2：派生投影，写路径当场落位，承载切片正文唯一副本"]
```
...
sequenceDiagram
    ...
    K->>I: 一趟提交攒下的增删（只提交，不回读文件）
    H->>K: embeddings().sync(space, batch)（批次结束后触发一次）
    K->>K: 按缺口调模型补齐，最后逐档核对缺口标成就绪（补齐不提交索引）
```

- 数据提交与索引更新分开：写操作当场把索引改到位（删摘、增写、改覆盖——文档/词条攒进 writer，不 commit）；索引由使用方在合适时机调用 `update_index()` 一趟提交。读取永不拿写锁、也不替写者提交，写入后未提交的部分暂不可检索，直到 `update_index` 或关闭收尾。
- 停电恢复靠主库标记：写操作在主库记录上留「正在写入 / 正在删除」状态位，开库时读标记、按主库现状把没写完的重新同步、没删完的删完，绝不重放当时的输入。标记残留最多换来一次幂等的廉价重算。索引与主库的差集对齐只留给下游显式调用的 `reconcile_index()`。
...
- 正文只读一次、只切一次：写入时就地切好，随文档一路带到索引；索引阶段不回读源文件。回源只发生在两处——向量化取切片正文（切片正文只存在索引里，写入侧提交索引后才读得到），以及停电恢复补切片文档（标记指示某几条没同步完，按主库现状回源重切）。
