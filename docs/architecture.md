...
- 笔记的路径只在 `notes` 表存一次（→strings）；标题由路径派生，整条路径进索引的路径列。`chunks` 不重复携带正文，只存 `note_id` 与行区间，切片正文按切片 ID 从索引取回。
...
- 正文只读一次、只切一次：写入时就地切好，随文档一路带到索引；索引阶段不再回读源文件。唯一需要回源的是重建（格式升级、索引损坏、上次写入未收尾）。
