...
| 层 | 载体 | 是否权威 | 说明 |
|---|---|---|---|
| 结构化数据 | SQLite `store.sqlite3` | 权威 | 记录、标签、图谱引用、笔记路径、向量、索引更新日志 |
| 全文索引 | Tantivy `text-v2/` | 投影，可重建 | 只服务检索，含可检索正文的 stored 副本；损坏或缺失可从 SQLite 与笔记文件重建 |
| 向量 | SQLite `embeddings` 表 | 权威 | 宿主注册模型回调，库内部生成并写回，按向量空间隔离 |

**SQLite 是唯一权威。** 全文索引是可重放的投影；写入只登记待办、不就地索引，由使用方择时调用 `update_index` 追平。索引落后或重建都不会让已提交的数据失效。
...
| 笔记 | `kb.notes()` | 文件路径与带行号的切片（正文在文件，库内不留副本） |
| 向量 | `kb.embeddings()` | 向量空间注册、模型回调注册、内部同步补齐、命名空间向量化开关 |
