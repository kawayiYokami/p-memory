...
| 全文索引 | Tantivy `text-v2/` | 投影，可重建 | 只服务检索，损坏或缺失可从 SQLite 重建 |
| 向量 | SQLite `embeddings` 表 | 权威 | 由宿主生成后写回，按向量空间隔离 |
...
| 向量 | `kb.embeddings()` | 向量空间注册、待向量化清单、向量写回 |
| 检索 | `kb.search(&req)` | 关键词、向量、混合检索 |
