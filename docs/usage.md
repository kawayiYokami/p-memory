...
let req = PresetRequest {
    ...
    filter: ReadFilter { namespace: "demo".into(), ..Default::default() },
    ..Default::default()
};
...
// 登记该知识领域的笔记根目录：写入的前提，之后写入的路径必须是它的子路径，库里存相对路径
kb.notes().set_root("demo", "./data/demo")?;
kb.notes().root("demo")?;              // -> Some("./data/demo")

// 只给路径：库自己读文件，标题取文件名（去扩展名），路径即身份
kb.notes().upsert_file(NoteFileInput::new("./data/demo/notes/characters/overview.md"))?;
```

登记根目录之后，库里存的是 `notes/characters/overview.md`。相对路径拆成目录段与文件名：目录段仍与调用方给的标签合并、写在每一条切片记录上供筛选；进索引时目录段进 `path` 列、文件名进 `name` 列，两者都只挂在第一片上。文件名带固定倍数加权、可被常规检索搜到；目录段不参与常规匹配，只在笔记预设「书名块不够」时兜底补位。没登记根目录的领域不允许写入笔记：写入直接报校验错，库从头到尾不接触上游绝对路径。
...
kb.embeddings().set_vectorization("demo", "notes", false)?;
