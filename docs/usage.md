...
登记根目录之后，库里存的是 `bwiki/沧州/澜川.md`。相对路径拆成目录段与文件名：目录段仍与调用方给的标签合并、写在每一条切片记录上供筛选；进索引时目录段进 `path` 列、文件名进 `name` 列，两者都只挂在第一片上。文件名带固定倍数加权、可被常规检索搜到；目录段不参与常规匹配，只在笔记预设「书名块不够」时兜底补位。没登记根目录的领域不允许写入笔记：写入直接报校验错，库从头到尾不接触上游绝对路径。
