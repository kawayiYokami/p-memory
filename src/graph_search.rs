...
use crate::types::{ReadFilter, RecordKey, RecordKind, WriteReceipt};
...
impl GraphStore {
    ...
    pub fn ego(&self, root: i64, depth: usize, filter: &ReadFilter, limit: usize) -> Result<Vec<Entity>> {
        ...
        let state = self.0.read()?;
        ids.into_iter().map(|id| storage::get(state.conn(), &RecordKey { id }, filter)).collect()
    }
    ...
    pub fn path(&self, from: i64, to: i64, filter: &ReadFilter) -> Result<Option<Vec<Entity>>> {
        ...
        Ok(Some(
            ids.into_iter()
                .map(|id| storage::get(state.conn(), &RecordKey { id }, filter))
...
mod tests {
    ...
    fn filter_isolates_namespaces() {
        ...
        let fa = ReadFilter { namespace: "a".into(), scopes: vec!["public".into()], tags: vec![] };
        let fb = ReadFilter { namespace: "b".into(), scopes: vec!["public".into()], tags: vec![] };
    ...
    fn same_as_contracts_alias_nodes() {
        ...
        let ents = kb
            .graph()
            .apply_batch(&GraphBatch { entities: vec![ent("林昭"), ent("林先生"), ent("导师")], ..Default::default() })
            .unwrap()
            .value
            .entities;
