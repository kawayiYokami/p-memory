...
impl KnowledgeBase {
    ...
    fn preset_graph(&self, query: &str, request: &PresetRequest, diagnostics: &mut SearchDiagnostics) -> Result<GraphSection> {
        ...
        // 与全文路同一套扩散：剩余词里写「beta」时，也带上「alpha」这一类同义谓词。
        ...
        // 事件这一批常常比预算大一到两个数量级（成员是高连接度实体时尤甚）：先按正文长度
        // 定下要留哪几条，再装配这几条。装配整批等于把它们全读出来解一遍再丢掉。
...
/// 与 `truncate_values_by_chars` 同一条规则（顺序不变、第一条无论多长都留下），
/// 只是长度由调用方先查好：要留的条数远小于取回的条数时，先定 id、再装配那几条。
