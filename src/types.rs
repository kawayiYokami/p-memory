...
pub struct ReadFilter {
    ...
    #[serde(default)]
    pub tags: Vec<String>,
    /// 只在这些笔记（notes 的记录 id）内检索。
    ///
    /// 空表示不限定，行为与不传该字段完全一致。给出时与 namespace / scopes / tags
    /// 叠加（与门），且没有笔记归属的记录（记忆、实体、关系、事件）不参与命中。
    #[serde(default)]
...
impl Default for ReadFilter {
    fn default() -> Self { Self { namespace: default_namespace(), scopes: default_scopes(), tags: vec![], note_ids: vec![] } }
}
...
pub struct SearchDiagnostics {
    ...
    /// 因重排回调声明的 `max_candidates` / `max_tokens_total` 预算而未送入重排的候选数。
